...
export class GatewayCore {
  constructor({ agents, defaultModel, sessions, agentPresets, workspaceRegistry, sessionPersistence, sessionTitle, log = console, settingsPath, statePath }) {
    ...
    this.autoHeal = true;
    /** 注入健康检查间隔（分钟，配置读）：autoHeal 开启时的检查周期。默认 5。 */
    this.healthIntervalMin = 5;
    /** 注入健康检查定时器（autoHeal 开启时挂起，关闭时清掉）。 */
  ...
  applyConfig(cfg) {
    ...
    if (cfg.autoHeal !== undefined) this.autoHeal = !!cfg.autoHeal;
    ...
    if (typeof cfg.autoHeal === "boolean" || cfg.healthIntervalMin !== undefined) {
  ...
  async startListener() {
    ...
    // 注入保障（autoHeal 开启时）：启动前确认注入在，异常先自动 launch 恢复；失败不阻塞 rpc 启动，定时检查会继续尝试。
    if (this.autoHeal) {
      const healthy = await this.checkInjectionHealth();
      if (healthy) this.log?.info?.(`imessage: 启动注入检查通过`);
      else this.log?.warn?.(`imessage: 启动时注入不可用，rpc 仍将启动，定时检查会继续尝试恢复`);
    }
    // 健康检查定时器：无论配置是否有 autoHeal 字段都要挂（构造函数默认 true，
  ...
  async checkInjectionHealth() {
    ...
    this._healing = true;
    try {
      this.log?.warn?.(`imessage: 注入检测异常，尝试自动恢复（${this.imsgCmd} launch）`);
      const launched = await this._launchInjection();
      if (!launched) {
        this.log?.error?.(`imessage: imsg launch 失败，请手动执行 ${this.imsgCmd} launch`);
        return false;
      }
      // 等 injection 完成
      await new Promise((r) => setTimeout(r, 5000));
      const recovered = await this._probeInjection();
      if (recovered) {
      ...
      this.log?.error?.(`imessage: 注入自动恢复失败（launch 后仍不可用），等待下一轮检查`);
      return false;
    } finally {
      this._healing = false;
    }
  ...
  /** spawn `imsg status --json`，返回 advanced_features 是否 true。 */
  _probeInjection() {
  ...
  _launchInjection() {
    return new Promise((resolve) => {
      const [cmd, ...prefix] = splitCmd(this.imsgCmd);
      const child = spawn(cmd, [...prefix, "launch"], { stdio: ["ignore", "pipe", "pipe"] });
      child.on("error", () => resolve(false));
      child.on("close", (code) => resolve(code === 0));
    });
  }

  /** 同步健康检查定时器：autoHeal 开 → 启动/重置（每 healthIntervalMin 分钟）；关 → 清除。 */
  syncHealthTimer() {
    if (this._healthTimer) { clearInterval(this._healthTimer); this._healthTimer = null; }
    if (!this.autoHeal) return;
    this._healthTimer = setInterval(() => {
      this.checkInjectionHealth().catch((e) => this.log?.warn?.(`imessage: 健康检查异常: ${e instanceof Error ? e.message : e}`));
  ...
  async _deliver(sender, workspace, message, generation) {
    ...
    try {
      ...
      const { text } = summarizeReply(agent.session.snapshotEvents(), firstSeq);
