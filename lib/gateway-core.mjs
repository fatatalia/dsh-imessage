...
export class GatewayCore {
  ...
  async checkInjectionHealth() {
    ...
    try {
      ...
      if (!recovered) {
        this.log?.error?.(`imessage: imsg launch 多次失败，请手动执行 ${this.imsgCmd} launch`);
        return false;
      }
  ...
  async _launchInjection() {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      await new Promise((resolve) => {
        const child = spawn("launchctl", ["asuser", String(uid), ...prefix, "launch"], { stdio: ["ignore", "pipe", "pipe"] });
        let out = "", err = "";
        child.stdout?.setEncoding("utf8");
        ...
        child.on("close", (code) => {
          this.log?.warn?.(`imessage: launch exit=${code} out=${out.trim().slice(0, 200)} err=${err.trim().slice(0, 200)}`);
          resolve();
        });
      });
      ...
      if (await this._probeInjection()) {
        if (attempt > 1) this.log?.warn?.(`imessage: imsg launch 第 ${attempt} 次成功（前次超时，重试生效）`);
        return true;
      }
      this.log?.warn?.(`imessage: imsg launch 第 ${attempt} 次后注入未生效${attempt < 3 ? "，重试" : "，放弃"}`);
    }
    return false;
  }
  ...
  syncHealthTimer() {
    ...
    this._healthTimer = setInterval(() => {
      this.checkInjectionHealth().then((ok) => {
        this.log?.info?.(`imessage: 注入健康检查（${this.healthIntervalMin}min 周期）: ${ok ? "正常" : "异常"}`);
