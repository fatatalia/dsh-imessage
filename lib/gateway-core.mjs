...
export class GatewayCore {
   ...
   * start 用 duration 模式（60s 自动消失，贴近原生体验，即使 stop 丢失也会自愈）；
  ...
  async setTyping(handle, on) {
    ...
    const p = this._typingChain.then(() =>
      this.safeRpc("typing", on ? { to: handle, duration: "60s" } : { to: handle, stop: true }),
    );
   ...
   * - `step/start`（模型推理开始）→ typing on，`step/end`（模型返回）→ typing off
   * - `assistant/message`（模型产出一条完整回复）→ 若开启流式发送则即时发出
   * 多轮 LLM（工具循环）自然交替；按 seq 增量消费，避免重复触发历史事件。
   */
  _syncStream(agent, sender) {
    ...
    for (const evt of agent.session.events) {
      ...
      else if (evt.type === "assistant/message" && this.streamReplies) this._sendReply(sender, evt);
  ...
  async deliver(sender, workspace, message) {
    ...
    // 流式事件消费轮询：typing 跟随模型调用 + （可选）assistant 消息即时发送。
    const typing = (await this.probeFeatures()).typing;
    let streamPoller = null;
    if (typing || this.streamReplies) {
    ...
    try {
      ...
      // 无条件兜底关闭 typing（setTyping 内部自带能力检查，探测失败/不支持时静默跳过）。
      // 不依赖入口探测变量 typing：入口探测失败但后续能力缓存可用时，on 已发出而 stop 会被
      // 这里无条件补上，避免 iOS 端 typing 指示残留。
