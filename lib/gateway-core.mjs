...
export class GatewayCore {
  constructor({ agents, defaultModel, sessions, agentPresets, workspaceRegistry, sessionPersistence, sessionTitle, log = console, settingsPath, statePath }) {
    ...
    /** turn 级单步超时（ms）：>0 时投递的 agent 挂 __pluginConfig.turnGuard.stepMs（dsh-turn-guard 读取）；0 = 不限制。 */
    this.stepTimeoutMs = 0;
  ...
  applyConfig(cfg) {
    ...
    if (cfg.stepTimeoutMs !== undefined) this.stepTimeoutMs = Number(cfg.stepTimeoutMs) > 0 ? Number(cfg.stepTimeoutMs) : 0;
  }
  ...
  async _deliver(sender, workspace, message) {
    ...
    // turn 级单步超时配置（dsh-turn-guard 读取）：从 settings 的 stepTimeoutMs 读值，
    ...
    const stepMs = this.stepTimeoutMs > 0 ? this.stepTimeoutMs : 0;
