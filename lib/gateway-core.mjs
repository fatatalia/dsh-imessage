...
/**
 * 会话重建关键词表（多语言）。与停止指令同一套匹配规则：整条消息规范化
 * （小写/去空白/去尾部标点）后**精确匹配**。命中即：中断当前轮 → 备份并归档
 * 当前会话 → 清空 sender 映射，由 _deliver 的归档迁移逻辑在下一条消息开启新会话。
 * 因为要求整条消息，口语短语出现在正常句子里不会误触发。
 */
export const DEFAULT_REBUILD_KEYWORDS = [
  // 中文
  "重建会话", "重开会话", "重新开会话", "重置会话", "新会话", "新建会话",
  "换个会话", "换会话", "重来",
  // 英文
  "rebuild session", "reset session", "new session", "restart session", "new chat", "reset chat",
];
...
/** 上下文压缩开始提示：压缩是几十秒的 LLM 调用，会让本轮明显变慢，提前说明原因。 */
const COMPACTING_NOTICE = '⏳ 上下文压缩中（本轮会稍慢）';

/**
 * compaction/end 的 error 是 errorChain(error) 产出的字符串（非结构化 code，见
 * dsh-compaction-basic compactSurfaceRegion），只能按子串归类成人话。未命中则原样透出。
 */
const COMPACTION_ERROR_HINTS = [
  ["is not smaller than the shadowed content", "摘要不比被压缩的内容小，上下文保持原样（压缩区间太窄，最常见）"],
  ["compaction lock is already active", "已有一次压缩在进行中，本次跳过"],
  ["already in progress", "已有一次压缩在进行中，本次跳过"],
  ["surface changed", "会话内容在压缩期间变动了，本次作废"],
  ["no open turn", "没有进行中的对话轮，压缩事件无处归属"],
  ["not a balanced boundary", "压缩切点会截断一次工具调用，已跳过本次压缩"],
  ["still above threshold after", "压缩后仍超过阈值，重试次数已用尽"],
  ["no context capacity", "该模型未配置上下文容量，无法判断压缩阈值"],
];

/** 把 compaction/end 的 error 字符串归类成人话；未命中则截断原样透出（保底仍可诊断）。 */
function describeCompactionError(raw) {
  const text = String(raw ?? "");
  for (const [needle, human] of COMPACTION_ERROR_HINTS) {
    if (text.includes(needle)) return human;
  }
...
export class GatewayCore {
  constructor({ agents, defaultModel, sessions, agentPresets, workspaceRegistry, sessionPersistence, sessionTitle, log = console, settingsPath, statePath }) {
    ...
    /** 压缩事件通知开关（配置读）：compaction/start 提示"正在压缩"，compaction/end 报压缩量或失败原因。 */
    this.compactionNotice = true;
    ...
    /** 会话重建关键词表（Set；配置可覆盖）。整条消息精确命中即归档当前会话并翻新（不要求 agent 忙碌）。 */
    this.rebuildKeywords = new Set(DEFAULT_REBUILD_KEYWORDS);
    /** 重建时是否先把会话目录备份到 ~/.dsh/sessions-backup/（配置可覆盖）。归档不可逆，默认开。 */
    this.rebuildBackup = true;
    ...
    /** 压缩事件配对暂存：compactionId → { count, tokens }。数据由 compaction/summary 提供，
     * 由 compaction/end 消费并删除；异常中断（无 end）时在下次 start 处顺手清掉同 session 陈旧项。 */
    this._compactionStats = new Map();
  ...
  applyConfig(cfg) {
    ...
    if (cfg.compactionNotice !== undefined) this.compactionNotice = !!cfg.compactionNotice;
    ...
    if (Array.isArray(cfg.rebuildKeywords)) {
      const kw = cfg.rebuildKeywords.map((k) => String(k).trim().toLowerCase()).filter(Boolean);
      if (kw.length > 0) this.rebuildKeywords = new Set(kw);
    }
    if (cfg.rebuildBackup !== undefined) this.rebuildBackup = !!cfg.rebuildBackup;
  ...
  /** 关键词规范化：小写 / 统一撇号 / 折叠空白 / 去尾部标点。停止与重建指令共用同一套规则。 */
  ...
  /** 停止指令检测：整条消息规范化后精确匹配停止词表（参考 OpenClaw abort-primitives）。 */
  isStopRequest(text) {
    return this.stopKeywords.has(this.normalizeKeyword(text));
  }

  /** 会话重建指令检测：整条消息规范化后精确匹配重建词表。与停止指令不同，不限 agent 忙碌状态。 */
  isRebuildRequest(text) {
    return this.rebuildKeywords.has(this.normalizeKeyword(text));
  }
   ...
   *
   * 形状陷阱（2026-09-22 定案）：`sessionPersistence.list()` 返回的是 **snapshot 数组**
   * `[{ header, revision, sizeBytes }]`，会话 id 在 `snapshot.header.id` 里，顶层**没有**
   * `id` 字段（见 `dsh-session-persistence-jsonl` 的 `list()`；官方消费方
   * `dsh-workspace` 也是 `list().map((snapshot) => snapshot.header)`）。
   *
   * 早期这里读的是 `h.id` —— 恒 `undefined`，于是判定**永远为 false**，配合下面的静默
   * catch，表现就是"每次重启都新建会话、历史归零"。这个 bug 与插件同龄，只是从不报错。
   */
  async isPersisted(id) {
    try {
      const snapshots = await this.sessionPersistence?.list?.();
      return !!snapshots?.some((snapshot) => {
        // 兼容两种形状：标准 snapshot（id 在 header 里）与扁平条目。
        const candidate = snapshot?.header?.id ?? snapshot?.id;
        return candidate !== undefined && String(candidate) === String(id);
      });
    } catch (error) {
      // 不再静默：查询失败与"确实不存在"是两回事，前者必须留痕，否则同类问题再次隐身。
      this.log?.warn?.(`imessage: isPersisted(${id}) 查询失败，按不存在处理: ${error instanceof Error ? error.message : error}`);
      return false;
    }
  }
  ...
  /**
   * 备份一个会话目录（归档不可逆，翻新前留底；备份本身失败不阻塞重建）。
   * 源 `~/.dsh/sessions/<工作区目录>/<会话 id>/` → 目标 `~/.dsh/sessions-backup/<会话 id>-<时间戳>/`。
   * 会话 id 不携带工作区信息，故遍历 sessions 下的工作区目录定位；找不到返回 null。
   * @returns {Promise<string|null>} 备份目标路径，未找到或失败为 null。
   */
  async backupSessionDir(sessionId) {
    try {
      ...
      for (const entry of buckets) {
        ...
        const dir = join(sessionsRoot, entry.name, String(sessionId));
        if (await stat(dir).then((s) => s.isDirectory()).catch(() => false)) { source = dir; break; }
      }
      if (!source) return null;
      const backupRoot = join(homedir(), ".dsh", "sessions-backup");
      const stamp = new Date().toISOString().replace(/[:.]/g, "-");
      const dest = join(backupRoot, `${sessionId}-${stamp}`);
      await mkdir(backupRoot, { recursive: true });
      // dest 必须不存在：Node 的 cp 在 dest 已存在时会把源复制成 dest/<源目录名>，多套一层。
      await cp(source, dest, { recursive: true });
      this.log?.info?.(`imessage: 已备份会话 ${sessionId} → ${dest}`);
      return dest;
    } catch (error) {
      this.log?.warn?.(`imessage: 备份会话 ${sessionId} 失败: ${error instanceof Error ? error.message : error}`);
      return null;
    }
  }

  /**
   * 重建当前 sender 的会话：中断在跑的轮次 → 备份并归档旧会话 → 清空 sender 映射。
   *
   * 归档**两个候选**（映射 id + 稳定 id）是必需的：_deliver 的候选顺序是
   * `[sessionMap[sender], sessionIdFor(sender)]`，只归档映射 id 会让它回退到那个
   * hash 派生的稳定 id 会话上，等于没重建。
   *
   * 归档后**不在这里创建新会话**：下一条消息交给 _deliver 既有的「归档迁移」逻辑
   * （候选全部归档/不存在 → 新建随机 id 并写回状态文件），那份逻辑还要处理
   * agentOptions、工作区归属、标题等，重复一份只会与它分叉。
   * @param {string} sender 发送者 handle。
   * @param {string} workspace 工作区路径（当前仅用于日志/未来扩展）。
   * @returns {Promise<{archived: string[], report: string}>}
   */
  async rebuildSession(sender, workspace) {
    const candidates = [...new Set([this.sessionMap[sender], this.sessionIdFor(sender)]
      .filter(Boolean)
      .map(String))];
    const archives = [];

    // 1. 先中断在跑的轮次：不能一边往旧会话写、一边把它归档掉。
    for (const id of candidates) {
      const running = this.agents.get(id);
      if (running?.status === "running") {
        this.log?.info?.(`imessage: 重建前中断运行中的会话 ${id}`);
        // 🐞 2026-09-24 修：cancel() 只接受封闭联合 AgentCancelCause
        //（{kind:'user'|'parent'|'hook'|'disposed'}），传裸 Error 会让
        // dsh-agent-loop 的 abortedCancelCause() 落到 default: assertNever() 抛异常。
        // 那个异常发生在 catch 块内部、不会再被捕获 → 冒泡到 finally 时 turnEnds
        // 仍是 null → 会话里落盘 {"type":"turn/end","reason":null} → 前端读
        // reason.kind 抛 TypeError → 整个会话页面白屏。重建=旧 agent 被处置。
        running.cancel({ kind: "disposed" });
        await running.whenIdle?.().catch(() => {});
      }
    }

    // 2. 备份 + 归档：跳过已归档的、跳过根本不存在的（避免往归档集里塞空 id）。
    for (const id of candidates) {
      if (this.isArchived(id)) continue;
      const live = !!this.agents.get(id);
      if (!live && !(await this.isPersisted(id))) continue;
      const backup = this.rebuildBackup ? await this.backupSessionDir(id) : null;
      await this.workspaceRegistry?.archiveSession(id);
      archives.push({ id, backup });
      this.log?.info?.(`imessage: 已归档会话 ${id}${backup ? `（备份 ${backup}）` : ""}`);
    }

    // 3. 清空映射：下一条消息由 _deliver 建全新会话并写回状态文件。
    delete this.sessionMap[sender];
    await this.saveState();

    const lines = ["🆕 会话已重建"];
    if (archives.length === 0) {
      lines.push("（没有找到可归档的旧会话）");
    } else {
      for (const { id, backup } of archives) {
        lines.push(`· 已归档 ${id}`);
        lines.push(backup ? `  备份：${backup}` : "  ⚠️ 备份未生成");
      }
    }
    lines.push("下一条消息将开启全新会话。");
    return { archived: archives.map((a) => a.id), report: lines.join("\n") };
  }
   ...
   * - `compaction/start`（上下文压缩开始，摘要调用之前落盘）→ 提示本轮为何变慢
   * - `compaction/summary`（压缩量数据）→ 暂存，供 end 组成结果文案
   * - `compaction/end`（压缩成功或失败，失败带 error 字符串）→ 报压缩量或人话原因
   * - `turn/end`（reason.kind=max-tokens）→ 输出被截断，流式补发截断提示
   * typing 不再按事件开关：由 startTyping/stopTyping 的 keepalive 统一管理（每 3s 续发）。
   * 全部走 _sendChain 串行链，故发送顺序与事件 seq 顺序一致。
   */
  _syncStream(agent, sender) {
    ...
    for (const evt of agent.session.snapshotEvents()) {
      ...
      else if (evt.type === "compaction/start" && this.compactionNotice) this._sendCompactionStart(sender, evt);
      else if (evt.type === "compaction/summary" && this.compactionNotice) this._recordCompactionSummary(evt);
      else if (evt.type === "compaction/end" && this.compactionNotice) this._sendCompactionEnd(sender, evt);
  ...
  /**
   * 压缩开始提示：压缩是几十秒的 LLM 调用，会让本轮明显变慢，提前说明原因。
   * `compaction/start` 在摘要调用之前落盘，所以这条提示是真·实时，不是事后补充。
   */
  _sendCompactionStart(sender, evt) {
    const id = evt?.data?.compactionId ?? "";
    this.log?.info?.(`imessage: 压缩开始提示 给 ${sender}（${id}）`);
    this._sendChain = this._sendChain
      .then(() => this.send(sender, COMPACTING_NOTICE))
      .catch((e) => this.log?.warn?.(`imessage: 压缩提示发送失败: ${e instanceof Error ? e.message : e}`));
  }

  /** 暂存 compaction/summary 的压缩量（项数与 tokens），供同 id 的 compaction/end 组文案。 */
  _recordCompactionSummary(evt) {
    const data = evt?.data ?? {};
    const id = typeof data.compactionId === "string" ? data.compactionId : "";
    if (!id) return;
    // 正常路径由 compaction/end 消费后删除；仅在异常中断（有 start 无 end）时残留，故设上限兜底。
    if (this._compactionStats.size > 256) this._compactionStats.clear();
    this._compactionStats.set(id, {
      count: Array.isArray(data.shadowedSeqs) ? data.shadowedSeqs.length : 0,
      tokens: Number(data.shadowedTokenCount) || 0,
    });
  }

  /**
   * 压缩结束：成功报压缩量，失败报归类后的人话原因。
   * error 是 errorChain 字符串（非结构化 code），归类见 describeCompactionError。
   */
  _sendCompactionEnd(sender, evt) {
    const data = evt?.data ?? {};
    const id = typeof data.compactionId === "string" ? data.compactionId : "";
    const stats = this._compactionStats.get(id);
    this._compactionStats.delete(id);
    const rawError = typeof data.error === "string" ? data.error.trim() : "";
    let text;
    if (rawError) {
      text = `⚠️ 上下文压缩失败：${describeCompactionError(rawError)}（本次未改动上下文）`;
      this.log?.warn?.(`imessage: 压缩失败 给 ${sender}: ${rawError.slice(0, 200)}`);
    } else if (stats && stats.count > 0) {
      text = `✅ 上下文已压缩：${stats.count} 项 / 约 ${stats.tokens} tokens`;
      this.log?.info?.(`imessage: 压缩完成提示 给 ${sender}: ${stats.count} 项 / ${stats.tokens} tokens`);
    } else {
      text = "✅ 上下文已压缩";
      this.log?.info?.(`imessage: 压缩完成提示 给 ${sender}（无 summary 数据，仅报成功）`);
    }
    this._sendChain = this._sendChain
      .then(() => this.send(sender, text))
      .catch((e) => this.log?.warn?.(`imessage: 压缩结果发送失败: ${e instanceof Error ? e.message : e}`));
  }
  ...
  deliver(sender, workspace, message) {
    ...
    if (busyAgent?.status === "running" && this.isStopRequest(message)) {
      ...
      // 🐞 2026-09-24 修：同 L536 —— 必须传 AgentCancelCause 对象，否则
      // abortedCancelCause() 抛 assertNever，导致 turn/end 的 reason 落盘为 null。
      busyAgent.cancel({ kind: "user" });
      this._deliverGeneration.set(sender, (this._deliverGeneration.get(sender) ?? 0) + 1);
      this.send(sender, "⏹ 已停止（已清理排队消息）").catch(() => {});
      return Promise.resolve(null);
    }
    // 会话重建指令：同样必须在入链之前拦截——排队的话要等前一条 deliver 跑完（可能几十秒），
    // 而它本身不投递给 agent：这条消息不进对话、不消耗轮次，命中即执行。
    // 与停止指令不同，**不要求 agent 忙碌**：任何时刻都可以翻新会话。
    if (this.isRebuildRequest(message)) {
      this.log?.info?.(`imessage: ${sender} 发送重建指令（${String(message).slice(0, 20)}），归档当前会话并翻新`);
      // 作废串行链上排队的旧消息：它们属于即将被归档的会话，投递只会写进一个用户再也看不到的会话。
      this._deliverGeneration.set(sender, (this._deliverGeneration.get(sender) ?? 0) + 1);
      this.rebuildSession(sender, workspace)
        .then((result) => this.send(sender, result.report))
        .catch((error) => this.send(sender, `⚠️ 会话重建失败：${error instanceof Error ? error.message : error}`))
        .catch(() => {});
      return Promise.resolve(null);
    }
    ...
    // 任务 settle 后递减排队计数（失败也清理）。
    // ⚠️ 2026-09-24 修复：原写法 `task.finally(cb)` 会把 task 的 rejection **原样传播**
    // 到它返回的那个 promise 上，而此处无人接收 —— 于是「iMessage 发『停止』」这类
    // 会话中断场景下，_deliver 抛出的 TypeError 虽已被 deliver 的 try/catch 捕获并打印
    // 「处理消息失败」，却仍以**未处理的 promise rejection** 二次升级为
    // `dsh: fatal load failure` → 进程退出 → launchd 重拉整个 dsh（用户看到"重启"）。
    // 改用 then(noop, noop) 先行吞错：清理逻辑照常执行，且不再泄漏 rejection。
    task.then(() => void 0, () => void 0).then(() => {
  ...
  async _deliver(sender, workspace, message, generation) {
    ...
    // compactionNotice 发压缩进度——三者都靠同一次轮询消费）。
    let streamPoller = null;
    if (this.streamReplies || this.toolCallReplies || this.compactionNotice) {
    ...
    try {
      ...
      if (agent?.status === "running" && this.isStopRequest(message)) {
        ...
        // 🐞 2026-09-24 修：同 L536 —— 用户停止必须传 { kind: "user" }。
      ...
      //
      // 🐞 2026-09-24 修（代码东报障「iMessage 发『停止』→ 整个 dsh 重启」）：
      // summarizeReply 在“停止指令中断、本轮没有 turn/end 事件”时会返回 reason = null，
      // 而原判断写的是 `reason !== undefined`——**放行了 null**，随后 `reason.kind` 抛
      // TypeError: Cannot read properties of null (reading 'kind')。该异常发生在 _deliver
      // 内且未被捕获，冒泡成 `dsh: fatal load failure` → 进程退出 → launchd KeepAlive 拉起，
      // 表现为“发个停止把 dsh 弄重启了”。1280 行的日志模板串里有第二处同样的裸解引用。
      // 修法：只从可选链取一次 kind，用它同时做判断与插值。
      const reasonKind = reason?.kind;
      if (reasonKind !== undefined && reasonKind !== "completed" && reasonKind !== "max-tokens") {
        const why = reason?.reason?.reason ?? reason?.reason?.kind ?? "";
        const detail = reasonKind === "aborted" && why ? `（${reasonKind}: ${why}）`
          : reasonKind === "interrupted" ? "（被中断）"
          : `（${reasonKind}）`;
        this.log?.warn?.(`imessage: ${sender} 会话中断 kind=${reasonKind}${why ? ` reason=${why}` : ""}，回送提示`);
      ...
      this.log?.info?.(`deliver 完成 id=${id} reply=${text?.length ?? 0}字 stream=${this.streamReplies}${truncated ? " truncated=max-tokens" : ""}${reasonKind !== undefined && reasonKind !== "completed" ? ` reason=${reasonKind}` : ""}`);
      return reply;
    } finally {
